...
module.exports = (io, players, field, blobs) => {
    io.sockets.on('connection', socket => {
        ...
        socket.on('start', (data) => {
            let player = new Player(data.x, data.y, data.r, data.b, data.bc, data.n, socket.id, 0, data.picked_skin)
            ...
            players[socket.id].score = parseInt(players[socket.id]['r']*2) // Set player score
        ...
        // Socket eats blob
        socket.on('eatBlob', (_data) => {
            // Set player score
            if (_data['player_score'] !== undefined) players[socket.id]['score'] = _data['player_score']
